...
if platform == "linux":
	...
	if not Path(ispc_root).is_dir():
		...
		http_extract(
			"https://github.com/ispc/ispc/releases/download/v1.17.0/" + zip_name,
			format="tar.gz",
		)
else:
...
if not Path(one_tbb_root).is_dir():
	...
	if platform == "linux":
		...
		http_extract(
			"https://github.com/oneapi-src/oneTBB/releases/download/2019_U8/"
			+ zip_name,
			format="tar.gz",
		)
	else:
		...
		http_extract(
			"https://github.com/oneapi-src/oneTBB/releases/download/2019_U8/" + zip_name
		)

if platform == "linux":
	cmake_args.append(
		"-DDEPENDENCY_TBB_LIBRARY=" + one_tbb_root + "/lib/intel64/gcc4.7/libtbb.so.2"
	)
else:
	cmake_args.append(
		"-DDEPENDENCY_TBB_LIBRARY=" + one_tbb_root + "/lib/intel64/vc14/tbb.lib"
	)
...
cmake_configure(
	"..",
	generator,
	["-DTBB_ROOT=" + one_tbb_root, "-DTBB_INCLUDE_DIR=" + one_tbb_root + "/include"],
)
...
if platform == "linux":
	if generator == "Ninja Multi-Config":
		cmake_args.append(
			"-DDEPENDENCY_OPENIMAGEDENOISE_LIBRARY="
			+ oidn_root
			+ "/build/"
			+ build_config
...
#mkdir("build", cd=True)
#reset_to_commit("4895ab22a6148603394e03d3a7cf6887034ba274")
...
if platform == "linux":
	...
	# This cannot pass the cmake configArgs variable.*
...
# if platform == "linux":
	# cp(
		# ocio_root + "/build/include/OpenColorIO/OpenColorABI.h",
		# ocio_root + "/include/OpenColorIO/",
	# )
...
# if platform == "linux":
	# if generator == "Ninja Multi-Config":
		# cmake_args.append(
			# "-DDEPENDENCY_OPENCOLORIO_LIBRARY="
			# + ocio_root
			# + "/build/src/OpenColorIO/"
			# + build_config
			# + "/libOpenColorIO.so"
		# )
	# else:
		# cmake_args.append(
			# "-DDEPENDENCY_OPENCOLORIO_LIBRARY="
			# + ocio_root
			# + "/build/src/OpenColorIO/libOpenColorIO.so"
		# )
# else:
	# cmake_args.append(
		# "-DDEPENDENCY_OPENCOLORIO_LIBRARY="
...
parser.add_argument(
	...
	const=True,
	default=False,
	help="Build the Cycles library (otherwise uses pre-built binaries).",
)
...
if build_all or args["build_cycles"]:
	...
	install_prebuilt_binaries(
		"https://github.com/Silverlan/UniRender_Cycles/releases/download/latest/"
	)
...
cmake_configure(
	...
	generator,
	[
		"-DTBB_ROOT=" + one_tbb_root,
		"-DTBB_INCLUDE_DIR=" + one_tbb_root + "/include",
		"-D NO_PTEX=1",
		"-D NO_DOC=1",
		"-D NO_OMP=1",
		"-D NO_TBB=1",
...
if platform == "linux":
	if generator == "Ninja Multi-Config":
		cmake_args.append(
			"-DDEPENDENCY_OPENSUBDIV_LIBRARY="
			+ subdiv_root
			+ "/build/lib/"
			+ build_config
			+ "/libosdGPU.a"
		...
		cmake_args.append(
			"-DDEPENDENCY_OPENSUBDIV_CPU_LIBRARY="
			+ subdiv_root
			+ "/build/lib/"
			+ build_config
			+ "/libosdCPU.a"
		...
		cmake_args.append(
			"-DDEPENDENCY_OPENSUBDIV_LIBRARY=" + subdiv_root + "/build/lib/libosdGPU.a"
		)
		cmake_args.append(
			"-DDEPENDENCY_OPENSUBDIV_CPU_LIBRARY="
			+ subdiv_root
			+ "/build/lib/libosdCPU.a"
		)
else:
	cmake_args.append(
		"-DDEPENDENCY_OPENSUBDIV_LIBRARY="
...
if platform == "win32":
	...
	cmake_args.append(
		"-DDEPENDENCY_CYCLES_GLOG_LIBRARY="
		+ glog_root
		+ "/build/"
		+ build_config
		+ "/glog.lib"
...
if platform == "win32":
	...
	cmake_args.append(
		"-DDEPENDENCY_GFLAGS_INCLUDE=" + gflags_root + "/build_files/include"
	)
	cmake_args.append(
		"-DDEPENDENCY_GFLAGS_LIBRARY="
		+ gflags_root
		+ "/build_files/lib/"
		+ build_config
		+ "/gflags_static.lib"
